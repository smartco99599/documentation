...
- :guilabel:`Colors`: Change the primary and secondary colors used throughout reports to highlight
  important elements. The default colors are automatically generated based on the colors of the
  logo.
...
- :guilabel:`Reset report`: to discard all changes made to the report and reset it to its standard
  version.
- generate and download a report :guilabel:`Print preview`.
...
.. tip::
   - The dashed rectangles represent **conditional blocks** (*if/else* statements). These are used
     to show/hide content based on specific conditions. Click on the block to view the conditions.
  ...
  - The text highlighted in blue represents dynamic text, i.e., default values replaced by field
    values when the report is generated.
...
.. _studio/pdf-reports/formatting:
...
- Static text, i.e., the text that's not highlighted in blue, which can be modified directly in the
  editor. To add static text to the report, type the text where you want it.
- Dynamic text (highlighted in blue), which is replaced by field values when the report is
  generated, e.g., the SO number or the quotation date.
...
.. spoiler:: Add a data table
   ...
   To add a table in XML, you need to know the name of the fields and objects you need to access and
   display. As an example, let's add a table that details the tags on a sales order:
...
.. spoiler:: Conditional blocks
   ...
   attribute to define the condition, which is then evaluated as `True` or `False`. The table is
   not displayed if there are no tags in the quotation.
